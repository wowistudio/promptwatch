import { Storage } from "@google-cloud/storage";
import { Duplex, Readable } from "stream";
import { Pool } from "./pool.js";

export const storageClient = new Storage({
    apiEndpoint: 'http://localhost:8065',
    projectId: 'fake-project',
    credentials: {
        type: 'service_account',
        private_key:
            '-----BEGIN PRIVATE KEY-----\\nNtBLSfcqPULqD+h7br9lEJio\n-----END PRIVATE KEY-----\n',
        client_email: '123-abc@developer.gserviceaccount.com',
    }
});
export const BUCKET_NAME = "csv-uploads";
const MAX_WORKERS = 3;

export class CSVUploader<TResult, TItem> {
    private pool: Pool<TResult, TItem>;
    private stream: Readable;
    private parser: Duplex;
    private onFinished: () => void;

    constructor(pool: Pool<TResult, TItem>, stream: Readable, parser: Duplex, onFinished: () => void) {
        this.pool = pool;
        this.stream = stream;
        this.parser = parser;
        this.onFinished = onFinished;
    }

    async start(): Promise<void> {
        this.parser.on("data", async (row: any) => {
            const result = this.pool.push(row);

            if (!result) {
                this.parser!.pause();
                await this.pool.waitForBufferSpace();
                this.pool.push(row);
                this.parser.resume();
            }
            this.pool.dispatch();
        });

        this.parser.on("end", async () => {
            await this.pool.shutdown();
            console.log("[end] ✅ Producer finished streaming CSV.");
            this.onFinished();
        });

        this.parser.on("error", (err: any) => {
            console.error("CSV parse error:", err);
        });

        this.stream.pipe(this.parser);
    }

    async shutdown(): Promise<void> {
        await this.pool.shutdown();
    }
}